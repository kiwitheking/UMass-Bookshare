...
exports.getProfile = function getProfile(username_, res_, req_)
{
    ...
    var list = sequelize.query(
        ...
        // Change ORDER BY:
        "Select listing.*, book.author, book.title, book.isbn13, book.version FROM listing INNER JOIN book ON book.isbn13=listing.isbn13 WHERE listing.username = \'" +username_+ "\' ORDER BY listdate desc;"
    );
    ...
    found.then(function(record){
        ...
        if (record == null) {
            ...
            lname: lastname, email: email, phone: phone, school: institution, wishList: wishList,selling: selling});
          }
    })
}    
...
{
        var found = sequelize.query("UPDATE user SET profilepic = \'" + imgpath_ + "\' WHERE username = \'" + username_ + "\';", UserTable);
        found.then(function(record){
                function_(record);
        });
}
