    ...
    found.then(function(record){
        ...
        if(record != null)
        {
            ...
            listValues = {
				...
				forrent : record.dataValues.forrent.trim(),
				rentprice : record.dataValues.rentprice.trim(),
				forsale : record.dataValues.forsale.trim(),
				sellprice : record.dataValues.sellprice.trim(),
				forborrow : record.dataValues.forborrow.trim(),
				available : ecord.dataValues.available.trim(),
				description : record.dataValues.description.trim()
            } ;
        }
        row = listValues;
        res.render('../views/editlisting',{ listid: row.listid, forrent: row.forrent, 
              rentprice: row.rentprice, forsale: row.forsale, sellprice: row.sellprice, forborrow: row.forborrow,
          		availabe: row.available, description: row.description});
    });
}
...
{
    var found = sequelize.query("update listing set forrent=\'" +forRent+ "\', rentprice=\'" +rentPrice+ "\', forsale=\'" +forSale+ "\', sellprice=\'" +sellPrice+ "\', forborrow=\'" +forBorrow+ "\', available=\'" +available+ "\' where listid=\'" +listid+ "\';");
    found.finally(function(){
...
exports.recentListing = function recentListing_(res_,req_)
{
	var found = sequelize.query(
		// Add version?
		"Select listing.listid,listing.username, listing.rentprice, listing.sellprice, listing.description, book.author, book.title, book.isbn13, book.version FROM listing INNER JOIN book ON book.isbn13=listing.isbn13 ORDER BY listdate desc;"
	);
...
exports.wishListing = function wishListing_(username_,res_,req_)
{
	var found = sequelize.query(
		// Add version?
		"Select book.author, book.title, book.isbn13, book.version, Wishlist.wishDate FROM Wishlist INNER JOIN book ON Wishlist.isbn13=book.isbn13 WHERE Wishlist.username = \'" + username_ +"\' ORDER BY wishDate desc;"
	);
